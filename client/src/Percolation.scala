...
case class Percolation(n: Int, m: Int, edges: Set[((Int, Int), (Int, Int))]) {
  ...
  val gridLines =
    (0 to m).map { (i) =>
      <line x1="0" y1={(i * yscale).toInt.toString} x2={(n * xscale).toInt.toString} y2={(i * yscale).toInt.toString} stroke="grey" stroke-width="1"></line>
    } ++
      ((0 to n).map { (j) =>
        <line x1={(j * yscale).toInt.toString} y1="0" x2={(j * yscale).toInt.toString} y2={(m * yscale).toInt.toString} stroke="grey" stroke-width="1"></line>
      })
  ...
  val edgeLines =
    for {
      ...
      <line x1={(x1 * xscale).toInt.toString} y1={(y1 * yscale).toInt.toString} x2={(x2 * xscale).toInt.toString} y2={(y2 * yscale).toInt.toString} stroke="black" stroke-width="2"></line>
