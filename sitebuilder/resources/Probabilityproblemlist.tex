...
\item (\underline{Do not need to submit this}) Let $k_{1}<k_{2}<\ldots <k_{m}=n$. Define $Y_{1}=X_{1}+\ldots +X_{k_{1}-1}$, $Y_{2}=X_{k_{1}}+\ldots +X_{k_{2}-1}$, \dots $Y_{m}=X_{k_{m-1}}+\ldots +X_{k_{m}-1}$. What is the distribution of $(Y_{1},\ldots ,Y_{m})$?
\eenu
...
$$
\Sigma=\mat{\sig_{1,1}}{\sig_{1,2}}{\sig_{2,1}}{\sig_{2,2}} := \mat{a}{c}{c}{b}^{-1}.
$$
...
\benu
\item The samples are drawn with replacement (i.e., draw a coupon, note the number, put the coupon back in the box, and draw again\dots).
